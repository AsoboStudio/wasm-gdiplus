...
namespace Gdiplus
{
	...
	//typedef void IStream; //The use of streams ins not supported by the underlying implementation
	...
	public:
		...
		Bitmap(const char* filename, BOOL useEmbeddedColorManagement);
		Bitmap(Graphics* graphics, const char* filename, BOOL useEmbeddedColorManagement = FALSE);
		Bitmap(Graphics* graphics, const WCHAR* filename, BOOL useEmbeddedColorManagement = FALSE);
		Status Load(Graphics* gfx);
#endif
		...
		virtual Bitmap* Clone() const
		{
			...
			GpImage* cloneImage = NULL;
			Status status = updateStatus(DLLExports::GdipCloneImage(
				(GpBitmap*)nativeImage, &cloneImage));
			if (status == GpStatus::Ok) {
...
#ifndef _MSFS_WASM_PROXY_GDIPLUS
		GpCachedBitmapHandle nativeCachedBitmap;
#endif
		Status lastStatus;
	};
	...
	public:
		...
		Font(const WCHAR* familyName, const WCHAR* fontPath, REAL emSize, INT style, Unit unit = Unit::UnitPixel, const FontCollection* fontCollection = NULL);
		Font(const char* familyName, const char* fontPath, REAL emSize, INT style, Unit unit, const FontCollection* fontCollection);
#endif
